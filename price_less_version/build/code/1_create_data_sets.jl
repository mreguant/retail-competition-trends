...
df1_agg = combine(groupby(df1, [:date,:year,:quarter,:market, :group,:regulated,:incumbent,:smartmeter]), :consumer => sum => :consumer)
...
df_com = unstack(df_com, [:date,:year,:quarter,:market,:smartmeter,:consumer_commercial,:consumer_market], :group, :consumer, renamecols = x -> Symbol("consumer_", x))
...
rename!(dfinal,:consumer_false => :consumer_inc,:consumer_true => :consumer_reg)
